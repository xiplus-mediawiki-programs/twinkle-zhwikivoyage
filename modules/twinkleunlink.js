 ...
 * Active on:              Non-special pages, except Wikivoyage:Graffiti_wall
 * Config directives in:   TwinkleConfig
 */

Twinkle.unlink = function twinkleunlink() {
	if (mw.config.get('wgNamespaceNumber') < 0 || mw.config.get('wgPageName') === wgULS('Wikivoyage:涂鸦墙', 'Wikivoyage:塗鴉牆')) {
		return;
	}
...
Twinkle.unlink.callback = function(presetReason) {
	...
	Window.setTitle(wgULS('取消链入', '取消連入') + (mw.config.get('wgNamespaceNumber') === 6 ? '和文件使用' : ''));
	Window.setScriptName('Twinkle');
	Window.addFooterLink(wgULS('Twinkle帮助', 'Twinkle幫助'), 'w:WP:TW/DOC#unlink');
	...
	form.append({
		...
		label: [
			wgULS('这个工具可以取消所有指向该页的链接（“链入”）', '這個工具可以取消所有指向該頁的連結（“連入”）') +
				(mw.config.get('wgNamespaceNumber') === 6 ? wgULS('，和/或通过加入<!-- -->注释标记隐藏所有对此文件的使用', '，和/或透過加入<!-- -->注釋標記隱藏所有對此文件的使用') : '') +
				'。比如，',
			node1,
			wgULS('将会变成', '將會變成'),
			node2,
	...
	var wikipedia_api = new Morebits.wiki.api(wgULS('抓取链入', '截取連入'), query, Twinkle.unlink.callbacks.display.backlinks);
	...
	wikipedia_api.statelem.status(wgULS('载入中…', '載入中…'));
...
Twinkle.unlink.callback.evaluate = function twinkleunlinkCallbackEvaluate(event) {
	...
	var unlinker = new Morebits.batchOperation(wgULS('取消链入', '取消連入') + (imageusage ? wgULS('与文件使用', '與文件使用') : ''));
	unlinker.setOption('preserveIndividualStatusLines', true);
	unlinker.setPageList(pages);
	var params = { reason: reason, unlinker: unlinker };
	unlinker.run(function(pageName) {
		var wikipedia_page = new Morebits.wiki.page(pageName, wgULS('在条目“', '在條目“') + pageName + wgULS('”中取消链入', '”中取消連入'));
		...
		backlinks: function twinkleunlinkCallbackDisplayBacklinks(apiobj) {
			...
			if (apiobj.params.image) {
				...
				if (!list.length) {
					...
					apiobj.params.form.append({
						...
						tooltip: wgULS('您可在Twinkle属性中更改这个，请参见[[WV:TWPREFS]]', '您可在Twinkle屬性中更改這個，請參見[[WV:TWPREFS]]')
					});
					if ($(xmlDoc).find('query-continue').length) {
						apiobj.params.form.append({
							type: 'div',
							label: wgULS('显示头 ', '顯示頭') + list.length.toString() + wgULS(' 个文件使用。', ' 個文件使用。')
						});
					}
					apiobj.params.form.append({
						type: 'button',
			...
			if (backlinks.length > 0) {
				...
				if ($(xmlDoc).find('query-continue').length) {
					apiobj.params.form.append({
						type: 'div',
						label: wgULS('显示头 ', '顯示頭') + list.length.toString() + wgULS(' 个链入。', ' 個連入。')
					});
				}
				apiobj.params.form.append({
					type: 'button',
	...
	unlinkBacklinks: function twinkleunlinkCallbackUnlinkBacklinks(pageobj) {
		...
		if (params.doImageusage) {
			...
			if (text === oldtext) {
				...
				summaryText = wgULS('注释出文件使用', '注釋出文件使用');
				oldtext = text;
			}
		}
		...
		if (params.doBacklinks) {
			...
			// did we actually make any changes?
			if (text === oldtext) {
				warningString = warningString ? wgULS('反链或文件使用', '反連結或文件使用') : wgULS('反链', '反連結');
			} else {
